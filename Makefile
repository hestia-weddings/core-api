.PHONY: test test-no-lint lint install run
...
test-no-lint:
	...
	trap cleanup INT TERM; \
	printf "\n$${DIM}"; line; printf "$${RESET}\n"; \
	printf "$${BOLD}  == Test run (no lint) ==$${RESET}\n"; \
	if [ "$$use_spinner" -eq 1 ]; then \
		(mvn clean test -DskipTests=false -Dcheckstyle.skip=true -Dspotbugs.skip=true -Dspotless.check.skip=true -Dspring.profiles.active=test > "$$tmp" 2>&1) & \
		pid=$$!; \
		spinner="|/-\\"; i=0; \
		while kill -0 $$pid 2>/dev/null; do \
			now=$$(date +%s%N); \
